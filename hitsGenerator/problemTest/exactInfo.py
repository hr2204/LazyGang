...
def getDropDown(solutionSlide):
    ...
    for i in range(0, len(temp)):
        ...
        temp[i] = temp[i].replace("R" + str(i + 1), "").strip()
        ...
        # res = str(result)
        AllRes = AllRes + "["+str(result)+"]"
        if i != len(temp)-1:
            AllRes = AllRes + ","
        # print AllRes.replace( "'",'"')
    return AllRes.replace( "'",'"')
...
for solution in allSolution:
    ...
    if images:
        ...
        if allSolution.index(solution) < numOfAE :
            ...
            imagePath = str(skillNum)+"/"+"math-g8-c"+str(skillNumber[0])+"-s"+str(skillNumber[0])+"-"+str(skillNumber[1])+"-p"+ str(allSolution.index(solution)) + "-img1.png"
