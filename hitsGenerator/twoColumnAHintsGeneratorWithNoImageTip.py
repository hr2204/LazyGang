...
filePath = "/Users/sma/LazyGang/hitsGenerator/Grade 7- Skill 10.2.txt"
data = ""
skill="10.2"
...
for i in range(0, len(finalAERight)):
    for j in range(0, len(finalAERight[i])):
        tempObj = {"hint_id": dataIndex, "row": [
            {
                "type": "img",
                "value": str(skill)+"/math-g7-c10-s10-2"+"-ap"+str(i+1)+"-h"+str(j)+"-img1.png",
                "show": True
            },
            {
...
for i in range(0, len(finalExeRight)):
    for j in range(0, len(finalExeRight[i])):
        print str(skill)+"/math-g7-c10-s10-2"+"-p"+str(i)+"-h"+str(j)+"-img1.png"
        tempObj = {"hint_id": dataIndex+10, "row": [
            {
                "type": "img",
                "value": str(skill)+"/math-g7-c10-s10-2"+"-p"+str(i+1)+"-h"+str(j+1)+"-img1.png",
                "show": True
            },
            {
                "type": "mathtex_wrapper",
